...
API_VERSION = "guard.storage-slo.io/replay-trace/v1alpha1"
...
def assess_trace(document: Dict[str, object], reference_group: Optional[str] = None) -> TraceAssessment:
    ...
    if not isinstance(document, dict):
        return TraceAssessment(["trace must be an object"], 0, 0, 0.0, False, False)
    ...
    if document.get("apiVersion") != API_VERSION or document.get("kind") != "ReplayTrace":
    ...
    offsets: List[int] = []
    required_sample_fields = {"offsetSeconds", "writeWaitMilliseconds", "waitValid", "managementPlaneHealthy"}
    ...
    compatible = wait_statistic == "p95" and aggregation == "none" and semantics.get("provenance") in {"observed", "derived"}
    independent = (
        not errors

...
- Non-production promotion evidence is generated by a separate validator, not
  the product binary. It binds an approved SHA-256 to fixed read-only commands,
  validates output/privacy in memory, exercises SIGTERM, and emits an
  identity-free summary. Fake validation never counts as host evidence.
